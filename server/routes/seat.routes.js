...
seatRouter.post("/reserve", async (req, res) => {
  try {
    ...
    const availableSeatsInOneRow = await seatModel
      .find({
        isBooked: false,
        seatNumber: { $lt: 8 },
      })
    ...
    if (availableSeatsInOneRow.length >= numSeats) {
      ...
      const availableSeatsNearby = await seatModel
        .find({
          isBooked: false,
          row: nextRow,
        })
      ...
      if (availableSeatsNearby.length >= numSeats) {
        ...
        const availableSeatsAnyRow = await seatModel
          .find({
            isBooked: false,
          })
          .sort({ row: 1, seatNumber: 1 })
        ...
        if (availableSeatsAnyRow.length >= numSeats) {
          ...
          const unreservedSeats = await seatModel
            .find({ isBooked: false })
            .limit(numSeats);
          ...
          if (unreservedSeats.length >= numSeats) {
            ...
            return res
              .status(400)
              .json({ error: "Not enough available seats" });
          }
        }
      }
    }
